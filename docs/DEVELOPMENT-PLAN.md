...
- [x] CPT 登録: `saai_faq` / `saai_kb` / `saai_glossary`（DESIGN.md 3.1 準拠）
...
- [x] `saai-knowledge/kb-toc` ブロック（`parse_blocks()` で h2/h3 抽出、アンカー付与、スクロールスパイ）
- [x] `saai-knowledge/breadcrumbs` ブロック（BreadcrumbList JSON-LD）
- [x] 見出しアンカー付与フィルター（本文側の h2/h3 に ID を保証）
- [x] KB 記事テンプレート（2カラム、ブロック / クラシック両対応）+ モバイル折りたたみ
- [x] カテゴリーアーカイブ・KB ハブテンプレート
- [x] タームメタ `saai_order`（ターム並び順）+ 管理 UI
- [x] ショートコード版ラッパー（`[saai_kb_sidebar]` 等）
...
- [x] FAQ 個別ページテンプレート（質問=h1・回答が直下・`QAPage` JSON-LD。AI可読性要件: DESIGN.md §7.1）
...
- [x] Markdown 出力: FAQ/KB/用語の個別ページを `?format=markdown` で提供（`saai_markdown_output` フィルター、キャッシュあり）
- [x] llms.txt 連携（第1層+第2層）: SEOプラグイン向け説明文アダプター（Yoast/Rank Math/AIOSEO — 実装は「セクション注入」ではなく「空の説明文を自プラグインの抜粋で埋める」。docs/DESIGN.md §7.2 参照）+ 自名前空間 Markdown インデックス（`/{kb-base}/llms.txt`）。ルート `/llms.txt` の自前生成（第3層）はバックログ
- [x] RAG エクスポート: REST `/saai-knowledge/v1/export`（jsonl/json、`modified_after` 増分、KB は h2 セクション配列付き）+ 管理画面 JSONL/CSV ダウンロード
...
- [ ] 紐づけ解決ロジックのユニットテスト（M5-2 で完了）、商品ページの E2E テスト
