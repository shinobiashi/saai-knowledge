...
- テーマ/SEO プラグインは本番テンプレート描画の前にコンテンツを投機的にレンダリングしうる（抜粋生成・メタデータ解析）。「1リクエスト1回だけ出力」系のリクエストスコープ状態を真偽値で消費すると、破棄された投機的レンダリングが本番出力を潰す。再提示可能なシグネチャ（正規化属性+文脈post）でクレームする（`Faq_List::claim_structured_data_slot()`）。また `wp_json_encode()` は不正 UTF-8 で false を返しうるため、シグネチャ・キー用途で `(string)` キャストしない（空文字衝突する）。
