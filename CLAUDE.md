...
- the_content フィルターを通さず本文を手動レンダリングする場合は core の順序を再現する: classic コンテンツは `WP_Embed::run_shortcode()`→`autoembed()`（priority 8 相当、wpautop 前）→wptexturize→wpautop→shortcode_unautop→do_shortcode→`wp_filter_content_tags()`（priority 12 相当）。`setup_postdata()` は `$post` 以外に9個のグローバル（`$id`, `$authordata`, `$currentday`, `$currentmonth`, `$page`, `$pages`, `$multipage`, `$more`, `$numpages`）を変更するため、復元はスナップショット方式で行う（直前の global post が null のケースは `setup_postdata()` では戻せない）。
