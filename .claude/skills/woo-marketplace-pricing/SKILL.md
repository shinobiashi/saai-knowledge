...
収益分配70/30は2026-07時点で公式ドキュメントにて確認済み。価格帯テーブルは実勢調査に基づくため、
提案前に本スキル同梱の `scripts/analyze_competitors.sh`（本スキルのディレクトリ
`.claude/skills/woo-marketplace-pricing/` 基準の相対パス）または WooCommerce.com の
該当カテゴリページで最新価格を確認すること。

## 関連スキル

- `woo-marketplace-submission` — 価格設定ルール（他チャネルとの整合、SaaS Billing API）の提出面
- `woo-marketplace-content` — 製品ページでの価格の見せ方
